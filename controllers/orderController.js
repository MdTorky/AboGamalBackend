...
exports.createOrder = async (req, res) => {
  try {
    const { customerName, email, phoneNumber, whatsappNumber, items, totalAmount, extraRequests, paymentMethod } = req.body
    ...
    // FIX 2: Send Response IMMEDIATELY. Don't wait for email.
    res.status(201).json({
      success: true,
      message: "Order created successfully",
      order,
    ...
    // This runs after the user already got their response
    const trackingUrl = `${process.env.FRONTEND_URL}/track/${trackingNumber}`

    const mailOptions = {
      from: { name: 'Shawarma Fahman', address: process.env.EMAIL_USER },
      to: email,
      subject: "Order Confirmation - Shawarma Fahman",
      html: `
        <h2>Thank you for your order!</h2>
        ...
        <h3>Order Details:</h3>
        <ul>
          ${items.map((item) => `<li>${item.name} x${item.quantity} - RM${(item.price * item.quantity).toFixed(2)}</li>`).join("")}
        </ul>
        <p><strong>Total: RM${totalAmount}</strong></p>
        ${extraRequests ? `<p>Special Requests: ${extraRequests}</p>` : ""}
        <p>We'll notify you when your order is ready!</p>
    ...
    // Send without 'await' so it doesn't block, just log errors if any
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    const io = req.app.get("socketio")
    if (io) {
      console.log("Broadcasting update for order:", order.trackingNumber)
      io.emit("order_status_updated", order)
    }

    // FIX 3: Send Response IMMEDIATELY. Stop the spinner on frontend.
    res.json({
      success: true,
      message: "Order status updated successfully",
      order,
    ...
    if (mailOptions) {
      // Send email in background. Do NOT await it.
      transporter.sendMail(mailOptions).catch(err => console.error("Email update failed:", err));
    }
    // --------------------------------------------------------
...
// ... Keep getAllOrders, getPastOrders, getAnalytics, trackOrder, clearDayOrders, deleteOrder exactly as they were ...
exports.getAllOrders = async (req, res) => {
  try {
    const orders = await Order.find({
      orderStatus: { $in: ["pending", "ready"] },
